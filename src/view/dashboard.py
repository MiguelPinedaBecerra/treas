...
def DashboardView(page, tarea_controller):
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.ListTile(
                            ...
                            subtitle=ft.Text(f"{t['description']}\nPrioridad: {t['prioridad']}"),
                            trailing=ft.Badge(content=ft.Text(t['estadp']), bgcolor=ft.Colors.ORANGE_300)
                        ), padding=30
                    )
                )
            )
    ...
    def add_task(e):
        success, msg = tarea_cotroller.guardar_nueva(user['id_usuario'], txt_titulo.value, "", "media", "trabajo")
        if success:
            txt_titulo.value = ""
            refresh()
